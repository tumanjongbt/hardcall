...
- Celestial brand lives in the dashboard only (tokens, Unbounded / DM Sans / IBM Plex Mono, event-horizon `public/logo.png`, `public/banner-celestial.png`). The event-horizon mark is locked; do not ship supernova, protostar, or the geometric amber H. Keep `hardcall` out of routes, DDL, and status enums.
